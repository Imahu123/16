      ...
      $('body').on('click', 'input[type="submit"]', function() {
        if($(this).parents('form').find('.tos_agree').length) {
          if ($(this).parents('form').find('.tos_agree').is(':checked')) {
            ...
            alert("Anda harus menyetujui syarat dan ketentuan untuk checkout.");
            return false;
          }
        } else {
          $(this).submit();
...
==============================================================================*/
  ...
  function refreshCart(cart) {
    ...
    if ($cartBtn && $cartBtn.length) {
      ...
      var cart_items_html = "";
      var $cart = $("#cart ul");
      var myimgrbo = //cdn.shopify.com/s/files/1/2277/3237/t/8/assets/ICON_Direct_from_Roaster-01.png?v=18159350014559269490;

      $cartBtn.text(value.replace(/[0-9]+/,cart.item_count));
      $cart.find('li:not(:first)').remove();

      if (cart.item_count == 0) {
        $cart.append('<li class="empty_cart">Keranjang kamu masih kosong</li>');
      } else {
      	cart_items_html += '<li class="nb_roast"> Kopi berlabelkan <span> Direct from Roaster </span> akan langsung dikirim oleh roaster masing-masing</li>';
        $.each(cart.items, function(index, item) {
          var line_id = index + 1;
          cart_items_html += '<li class="cart_item">' +
            '<p class="mm-counter" id="updates_' + item.id + '">' +
          ...
          cart_items_html += '<div class="item_info"><h1>' + item.title;

          if(item.properties) {
            $.each(item.properties, function(title, value) {
              if (value) {
                cart_items_html += '<div class="line-item">' + title +': ' + value + '</div></h1>';
              }
            });
          }

          cart_items_html += '<div class="vendor"><p>Roaster : <span itemprop="brand">' + item.vendor + '</span></p></div>';
          
          cart_items_html += '<div class="label_rbo">' ; 
          	var tags = item.properties._producttags
            if(tags.indexOf("Direct from Roaster") > -1) {
               cart_items_html += '<img src="//cdn.shopify.com/s/files/1/2277/3237/t/8/assets/ICON_Direct_from_Roaster-01.png?v=18159350014559269490" alt="" />';
            }
          cart_items_html += '</div>';
        ...
        cart_items_html += '<li class="mm-label">' +
          ...
          '<a href="/cart" class="action_button edit_cart right">Lihat keranjang</a>' +
          '</li>';
    ...
    $('body').on('submit', '.product_form', function(e) {
      ...
      $.ajax({
        ...
        error: function(XMLHttpRequest) {
          ...
          $addToCartBtn.removeAttr('disabled').removeClass('disabled');
          $addToCartBtn.find('span').text("Tambah Ke Keranjang").removeClass('zoomOut').addClass('zoomIn');
        }
      });
